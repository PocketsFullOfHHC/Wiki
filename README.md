...
1. **前后端分页功能整合**：
    * 首页不需要分页，管理界面需要分页
    * 去除掉首页的reactive数据绑定，只使用ref方法
2. **制作电子书编辑表单**：
    * 点击编辑按钮时，弹出模态框
    * 模态框显示电子书表单，表单中使用响应式数据显示该行电子书基本信息
3. **完成电子书编辑功能**：
    * 增加后端保存接口
    * 前后端整合：点击保存时，调用保存接口
    * 保存成功刷新列表
    * 分离参数：保存接口请求参数用EbookSaveReq，查询接口请求参数EbookQueryReq，查询接口返回参数用EbookQueryResp
4. **完成电子书新增功能**：
    * 增加雪花算法工具类SnowFlake生成id
    * 完善后端保存接口并和前端新增功能整合
5. **增加删除电子书功能**：
    * 后端增加删除接口
    * 前端点击删除按钮时调用后端删除接口
    * 删除显示气泡确认框
    * 解决前后端交互时Long类型精度的bug问题：用JacksonConfig实现
6. **集成Validation，为列表接口增加参数校验**：
    * 集成spring-boot-starter-validation
    * 对保存接口和查询接口增加参数校验
        * 实体增加校验注解
        * 接口参数增加注解@Valid，开启校验
    * 增加校验不通过时的前端错误提示：全局异常处理，用ControllerExceptionHandler实现
7. **电子书管理功能优化**：
    * 增加关键字查询
    * 解决修改表单图书列表实时变化的bug：通过tool.ts实现在编辑时复制对象
8. **分类表设计与代码生成**：
